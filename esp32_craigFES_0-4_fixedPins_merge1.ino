...
void loop(){
  ...
  if(onOff){
    ramp(amp1, dac1, offTime1, rampTime1, onTime1);
    ramp(amp2, dac2, offTime2, rampTime2, onTime2);
  }
...
// ----- 
  if(Serial.available()) {
    ...
    switch(cmd) {
      ...
      case 'r':
        Serial.println("testing ramp");
        ramp(255, dac1, 2, 5, 10); // amp value, dac pin, off time, ramp time, on time
        Serial.println("finished ramp test");
      break;
...
void ramp(int amp, int dac, int offTime, int rise, int onTime){ // amp value, dac pin, desired rise time
  Serial.println("beginning ramp function . . . ");
  int current = 0;
  dacWrite(dac, 0); // turn dac off at start
  delay(offTime*1000);
  while(current != amp){
    current++;
    dacWrite(dac, current);
    delay(rise*1000/amp);
  }
  delay(onTime*1000);
  while(current != 0){
