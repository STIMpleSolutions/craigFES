...
volatile bool modeButtonArray[8];
volatile bool modeButtonResult;
volatile bool onOffArray[8];
volatile bool onOffResult;
volatile bool stimToggle;
...
void loop(){
  ...
  if(modeSwitch == true || stimToggle == true){
    printScreen();
    modeSwitch = false;
    stimToggle = false;
  }
  ...
  switch(modeNumber){ // beginning of a switch which evaluates the modeNumber variable
    ...
    case 2: // channel 1 duty
      ...
      if(digitalRead(downButton) == false && duty1 > 245){
          duty1--;
          Serial.println(duty1);
      }
      if (duty1old != duty1){
    ...
    case 8: // channel 2 duty
      ...
      if(digitalRead(downButton) == false && duty2 > 245){
          duty2--;
          Serial.println(duty2);
      }
      if (duty2old != duty2){
...
void startStopStim(){
  bool buttonBounceStopped = false;
  bool firstValue;
  while(!buttonBounceStopped){
    for(int i=0; i<8; i++){
      onOffArray[i]=digitalRead(startButton);
      delay(100);
    }
    firstValue = onOffArray[0];
    buttonBounceStopped = true;
    for(int i = 1; i < 8; i++) {
      if (onOffArray[i] != firstValue) {
        buttonBounceStopped = false;
      }
    }
  }
  if (!firstValue) {
...
void changeMode(){
  ...
  while(!buttonBounceStopped){
    for(int i=0; i<8; i++){
      modeButtonArray[i]=digitalRead(modeButton);
      delay(100);
    }
    firstValue = modeButtonArray[0];
    buttonBounceStopped = true;
    for(int i = 1; i < 8; i++) {
      if (modeButtonArray[i] != firstValue) {
        buttonBounceStopped = false;
      }
    }
  }
  if(onOff == false && !firstValue) {
